		...
		while 1
			...
			if (!empty(match))
				if (index(g:blacklist, match) == -1)
					if(has_key(cache,match)==1)
						execute 'execute 'syn keyword _semantic' . cache[match]. " containedin=phpBracketInString,phpVarSelector,phpClExpressions,phpIdentifier " . match
					else
